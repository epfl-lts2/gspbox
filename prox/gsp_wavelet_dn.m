...
%   setting down to zero the low pass filter of the filerbank w. w' is
%   w without the first filter.
%
...
Wop = @(x) gsp_filter_analysis(G, Wh, x);
Wtop = @(x) gsp_filter_synthesis(G, Wh, x);
...
switch param.method
    case 'FISTA'
        param_solver.gamma = min(1/(2*B^2),0.5);
...
sol = gsp_filter_synthesis(G,Wh,denoise_y_wav2_c)+y_noise_bf;
